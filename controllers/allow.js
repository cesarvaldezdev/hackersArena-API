...
class AllowCtrl {
  ...
  constructor() {
    ...
    this.get = async (req, res) => {
      // FIXME Agregar manejo de errores
      const data = await Allow.get(req.params.allowId);
      if (data.length === 0) {
    ...
    this.create = async (req, res) => {
      // FIXME Agregar manejo de errores
      const data = await new Allow({ id: req.params.allowId, name: req.body.name }).save();
    ...
    this.delete = async (req, res) => {
      // FIXME Agregar manejo de errores
      const data = await new Allow({ id: req.params.allowId }).delete();
      if (data === 0) {
